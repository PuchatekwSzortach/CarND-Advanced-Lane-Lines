...
class LaneLineFinder:
    ...
    def get_lane_equation_using_prior_knowledge(self, recent_lane_equations, logger):
        ...
        latest_equation = np.polyfit(center_points[:, 1], center_points[:, 0] + self.offset, deg=2) \
            if len(center_points) > 0 else recent_lane_equations[-1]
...
class MyCarImagePreprocessor:
    """
    Class for preprocessing images to make task of lane finding easier. Adapted to work best for my car
    """

    def __init__(self, parameters, warp_matrix):
        """
        Constructor
        :param parameters: dictionary with parameters for various preprocessing stages
        """
        ...
        self.shadow_preprocessor = MyCarShadowPreprocessor()
    ...
    def get_preprocessed_image(self, image):
        ...
        warped = self.get_warped_image(image)
        deshadowed = self.shadow_preprocessor.get_image_without_shadows(warped)
    ...
    def get_cropping_mask(self, image_shape):
        ...
        mask_coordinates = [
            [100, image_shape[0]],  # left bottom corner
            [image_shape[1] - 100, image_shape[0]],  # right bottom corner
            [int(0.7 * image_shape[1]), 0],  # right upper corner
            [int(0.3 * image_shape[1]), 0],  # left upper corner
        ]
...
def get_perspective_transformation_source_coordinates_my_car(image_shape):

    coordinates = np.array([
        [60, image_shape[0] - 60],  # left bottom
        [image_shape[1] - 40, image_shape[0] - 60],  # right bottom
        [image_shape[1] - 200, 240],  # right top
        [300, 240],  # left top
    ])
...
def get_perspective_transformation_destination_coordinates_my_car(image_shape):

    coordinates = np.array([
        [100, image_shape[0]],  # lower left corner
        [image_shape[1] - 100, image_shape[0]],  # lower right corner
        [image_shape[1] - 100, 0],  # upper right corner
        [100, 0]  # upper left corner
    ])

    return coordinates.astype(np.float32)


class MyCarShadowPreprocessor:
    """
    Class for removing shadows fitted for my car
    """
...
class MyCarSmoothVideoProcessor:
    """
    Video processor that searches for lane lines. A history of recent results is used when searching for lane lines.
    Fitted for my car
    """
    ...
    def get_image_with_lanes(self, image):
        ...
        left_finder = MyCarLaneLineFinder(mask[:, :mask.shape[1] // 2], offset=0)
        right_finder = MyCarLaneLineFinder(mask[:, (mask.shape[1] // 2):], offset=mask.shape[1] // 2)
